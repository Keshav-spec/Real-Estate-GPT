...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    .querySelectorAll(".about-content > div, .contact-content > div")
    .forEach((el) => {
      observer.observe(el);
    });
});
  ...
  card.innerHTML = `
        ...
        <div class="property-info">
            ...
            <div class="property-price">
                <h4>₹${(property.price / 10000000).toFixed(2)} Cr</h4>
...
function updateMarketStats(data) {
  ...
  document.querySelector(
    ".stat-value"
  ).textContent = `₹${data.average_price}/sqft`;
  document.querySelector(
    ".stat-change.positive"
  ).textContent = `+${data.price_change}% YoY`;

  // Update properties listed
  document.querySelectorAll(".stat-value")[1].textContent =
    data.properties_listed.toLocaleString();
  document.querySelectorAll(
    ".stat-change.positive"
  )[1].textContent = `+${data.listings_change}% MoM`;
      ...
      if (data.properties && data.properties.length > 0) {
        ...
        data.properties.forEach((property) => {
          ...
          propertyCard.innerHTML = `
                        ...
                        <p><i class="fas fa-rupee-sign"></i> ${property.price.toLocaleString(
                          "en-IN"
                        )}</p>
                        <p><i class="fas fa-vector-square"></i> ${
                          property.area
                        } sq ft</p>
                        <p><i class="fas fa-home"></i> ${property.bhk} BHK</p>
                        ${
                          property.amenities
                            ? `<p><i class="fas fa-star"></i> ${property.amenities.join(
                                ", "
                              )}</p>`
                            : ""
                        }
