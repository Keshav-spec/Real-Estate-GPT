            ...
            try:
                if isinstance(price, str):
                    ...
                    if 'cr' in price or 'crore' in price:
                        ...
                        return float(re.search(r'([\d.]+)', price).group(1)) / 100  # Convert lakhs to crores
                return float(price) / 100  # Convert lakhs to crores
            except:
                return np.nan
...
def get_properties():
    ...
    # Convert rupees to crores for comparison
    min_price_crores = min_price / 10000000
    max_price_crores = max_price / 10000000 if max_price != float('inf') else float('inf')
    
    filtered_df = filtered_df[
        (filtered_df['Price'] >= min_price_crores) &
        (filtered_df['Price'] <= max_price_crores) &
...
def get_market_data():
    ...
    avg_price = df['Price'].mean()
    avg_price_per_sqft = (df['Price'] * 10000000 / df['Size']).mean()
    ...
    for location in df['Location'].unique():
        ...
        location_stats[location] = {
            ...
            'avg_price_per_sqft': (location_df['Price'] * 10000000 / location_df['Size']).mean(),
    ...
    for bhk in df['Bedrooms'].unique():
        ...
        bhk_stats[f"{bhk}BHK"] = {
            ...
            'avg_price_per_sqft': (bhk_df['Price'] * 10000000 / bhk_df['Size']).mean(),
            'total_properties': len(bhk_df)
        }
    
    return jsonify({
        'market_overview': {
            'average_price': f"{avg_price:.2f} Cr",
...
def recommend_properties():
    ...
    filtered_df = df[
        (df['Price'] <= budget_crores * 1.1) &  # Allow 10% above budget
...
def get_property_details(id):
    ...
    # Format the response
    property_data['Price'] = f"{property_data['Price']:.2f} Cr"
    property_data['Size'] = f"{property_data['Size']:.0f} sq. ft."
    property_data['Price_Per_SqFt'] = f"₹{property_data['Price_Per_SqFt'] * 10000000:.2f}"
...
def find_similar_properties(property_id):
    ...
    for prop in similar_properties:
        prop['Price'] = f"{prop['Price']:.2f} Cr"
        prop['Size'] = f"{prop['Size']:.0f} sq. ft."
        prop['Price_Per_SqFt'] = f"₹{prop['Price_Per_SqFt'] * 10000000:.2f}"
...
def get_personalized_recommendations(df):
    ...
    # Premium recommendations (3+ BHK, High-end locations)
    premium_props = df[
        (df['Location'].isin(premium_locations)) & 
        (df['Bedrooms'] >= 3)
    ].nlargest(2, 'Price')
    
    # Sample premium properties with realistic values
    sample_premium = [
        {
            'location': 'Adyar',
            'price': 250,  # 2.5 Cr
            'bhk': '4 BHK',
            'size': 2400,
            'amenities': ['Indoor Games', "Children's Play Area", 'Library', 'Swimming Pool'],
            'highlights': [
                'Prime location with excellent connectivity',
                'Premium construction with high-end finishes',
                'Resort-style amenities and clubhouse',
        ...
        },
        {
            ...
            'bhk': '3 BHK',
            'size': 1850,
            'amenities': ['Yoga Deck', 'Multipurpose Hall', '24/7 Power Backup', 'Gym'],
            'highlights': [
                'Prime location with excellent connectivity',
                'Premium construction with high-end finishes',
                'Resort-style amenities and clubhouse',
    ...
    recommendations['premium'] = sample_premium
    
    # Sample mid-range properties
    sample_mid_range = [
        {
            ...
            'bhk': '3 BHK',
            'size': 1450,
            'amenities': ['Gym', 'Garden', 'Security', 'Covered Parking'],
            'highlights': [
                'Excellent value for money',
                'Well-connected to schools and hospitals',
                'Modern amenities with low maintenance',
                'Growing neighborhood with good appreciation potential'
            ]
        },
        {
            'location': 'Padur',
            ...
            'bhk': '2 BHK',
            'size': 1250,
            'amenities': ['Power Backup', 'Children Park', 'Security', 'Visitor Parking'],
            'highlights': [
                'Excellent value for money',
                'Well-connected to schools and hospitals',
                'Modern amenities with low maintenance',
    ...
    recommendations['mid_range'] = sample_mid_range
    
    # Sample affordable properties
    sample_affordable = [
        {
            'location': 'Siruseri',
            'price': 45,  # 45L
            ...
            'highlights': [
                'Best price in the locality',
                'Good rental potential',
                'Basic amenities included',
                'Upcoming area with infrastructure development'
            ]
        },
        {
    ...
    recommendations['affordable'] = sample_affordable
